...
if(isset($_POST['update_profile'])){
   ...
   print_r(json_encode($update_profile));

   $image = $_FILES['image']['name'];
   $image = filter_var($image, FILTER_SANITIZE_STRING);
   $image_size = $_FILES['image']['size'];
   $image_tmp_name = $_FILES['image']['tmp_name'];
   $image_folder = 'uploaded_img/user_images'.$image;
   ...
   if(!empty($image)){
      if($image_size > 2000000){
         ...
         if($update_image){
            ...
            unlink('uploaded_img/user_images/'.$old_image);
   ...
   if(!empty($update_pass) AND !empty($new_pass) AND !empty($confirm_pass)){
      if($update_pass != $old_pass){
         ...
         print_r(json_encode($update_pass_query));
...
<section class="update-profile">
   ...
   <form action="" method="POST" enctype="multipart/form-data">
      <img src="uploaded_img/user_images/<?= $fetch_profile['image']; ?>" alt="">
      <div class="flex">
         <div class="inputBox">
            <span>username :</span>
